...
const Email = () => {
  const { emails, loading, fetchEmailDetails,setFrom,setTo,setSubject, setEmail, setName, setthreadid } =
    useEmailContext();

  const handleClick = async (threadId, email, name,fromEmail,toEmail,subject) => {
    try {
        setEmail(email);
        setName(name);
        ...
        setTo(toEmail);
        setSubject(subject);
        console.log(threadId, email, name,fromEmail,toEmail,subject)
  ...
  useEffect(() => {
    if (emails.length > 0) {
      ...
      setFrom(emails[0].fromEmail);
      setTo(emails[0].toEmail);
        ...
        <ul>
          {emails.map((email, index) => (
            <li
              ...
              onClick={() =>
                handleClick(email.threadId, email.fromEmail, email.fromName,email.fromEmail,email.toEmail,email.subject)
